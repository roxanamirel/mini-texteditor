...
public class RecorderEngineTest {
	...
	@Test
	public void shouldNotPlayWithoutStartRecording(){
		recorderEngine.replay(history);
		verify(mockedEditorEngine, times(0)).engineInsert(TEST_STRING);
		verify(mockedEditorEngine, times(0)).engineSelect(START, END);
		verify(mockedEditorEngine, times(0)).engineCopy();

		verify(mockedEditorEngine, times(0)).engineSelect(TEST_STRING.length(),
				TEST_STRING.length());
		verify(mockedEditorEngine, times(0)).enginePaste();

		verify(mockedEditorEngine, times(0)).engineCut();
		verify(mockedEditorEngine, times(0)).engineSelect(0, 0);
	}
